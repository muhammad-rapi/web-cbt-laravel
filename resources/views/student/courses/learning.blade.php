            ...
            <div class="nav flex items-center w-full h-[92px] max-w-[1280px] mx-auto justify-between p-5">
                <div class="flex items-center gap-4">
                    <div class="w-[50px] h-[50px] flex shrink-0 overflow-hidden rounded-full">
                        <img src="{{asset(Storage::url($course->cover))}}" class="object-cover"
                            alt="thumbnail">
                    </div>
           ...
           @if ($errors->any())
                <ul>
                    @foreach ($errors->all() as $error)
                        <li class="py-5 px-5 bg-red-700 text-dark">{{ $error }}</li>
                    @endforeach
                </ul>
            @endif
        <form class="learning flex flex-col gap-[50px] items-center mt-[50px] w-full pb-[30px]" method="POST" action="{{route('dashboard.learning.course.answer.store', ['course' => $course->id, 'question' => $question->id])}}">
            @csrf
            ...
            <div class="flex flex-col gap-[30px] max-w-[750px] w-full">
                ...
                <label for="{{$index}}"
                    ...
                    <input type="radio" name="answer_id" id="{{$index}}" value="{{$answer->id}}" class="hidden">
                </label>
                @endforeach
            </div>
            <button type="submit"
